...
#         # Language Dropdown completely removed. Forced to English natively.
...
#         if not source_type:
#             st.warning("Please enter a YouTube URL or upload a video file before summarizing.")
#         else:
...
#                 # Removed Language variable from generate_ai_summary call
...
#                 # Hardcoded "English" to satisfy the SQLite database 'language' schema requirement
...
#                 # Provide a clean, sanitized error rather than a raw Python stack trace
#                 error_msg = str(e)
#                 if "413" in error_msg or "rate_limit" in error_msg.lower():
#                     st.error("Groq Rate Limit Hit: Please wait a few seconds and try again, or check API keys.")
#                 elif "404" in error_msg:
#                     st.error("Model Not Found: Please verify your LLM_MODEL in .env is active on Groq.")
#                 else:
...
# Main Navigation Tabs (Updated Labels)
tab_summarize, tab_history = st.tabs(["🎬 Summarize Video", "🕘 History"])
...
with tab_summarize:
    ...
    if 'active_summary' in st.session_state:
        ...
        # Export Controls (Updated Labels)
        st.markdown("### 📤 Export Summary")
        ex_col1, ex_col2 = st.columns(2)
        ...
        with ex_col2:
            pdf_path = export_summary_pdf(meta, summary_res)
            with open(pdf_path, "rb") as f:
                if st.download_button("📄 Export as PDF", data=f.read(), file_name=f"{meta.title}.pdf", mime="application/pdf"):
                    increment_export_count(rec_id)
