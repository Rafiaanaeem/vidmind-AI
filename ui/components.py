...
def render_hero_header():
    ...
    with col1:
        ...
        <div class="hero-container">
            <div class="hero-title">AI Video <span>Summarizer</span></div>
            <div class="hero-subtitle">Summarize long videos into short, meaningful insights in seconds.</div>
        </div>
        ''', unsafe_allow_html=True)
    with col2:
        st.markdown("""
        <div style="text-align: right; font-size: 3.5rem; line-height: 1;">
            🎬✨
        </div>
        """, unsafe_allow_html=True)
...
def render_stats_cards(stats: Dict[str, Any]):
    ...
    with c1:
        ...
        <div class="stat-card-modern">
            ...
            <span class="stat-badge">▲ 30% this week</span>
        </div>
        """, unsafe_allow_html=True)
    with c2:
        st.markdown(f"""
        <div class="stat-card-modern">
            ...
            <div class="stat-number">{stats['time_saved_hrs']} <span style="font-size: 1rem;">hrs</span></div>
            <div class="stat-label">Time Saved</div>
            <span class="stat-badge">▲ 25% this week</span>
        </div>
        """, unsafe_allow_html=True)
    with c3:
        st.markdown(f"""
        <div class="stat-card-modern">
            ...
            <span class="stat-badge">High Quality</span>
        </div>
        """, unsafe_allow_html=True)
    with c4:
        st.markdown(f"""
        <div class="stat-card-modern">
            ...
            <span class="stat-badge">▲ 40% this week</span>
        </div>
        """, unsafe_allow_html=True)
...
def render_chapters(summary: SummaryResult):
    ...
    for c in summary.chapters:
        st.markdown(f"<p><b>{c.timestamp}</b> &nbsp;&nbsp; <b>{c.title}</b> — <span style='color:#64748B;'>{c.description}</span></p>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
